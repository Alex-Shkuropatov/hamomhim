<template>
  ...
  <modal v-if="$store.getters['modals/phoneCode/isOpened']" @close="closeB"  >

    <third-modal v-if="modal===0" v-bind="code"  @send='onThird' />

  <alert v-if="modal===1" v-bind="modalContent"  @send='onFourth'   />
...
<script>
  ...
  export default {
    ...
    methods: {
      closeB() {
        this.$store.commit('modals/phoneCode/close');
        this.modal = 0;
      ...
      onThird(data){
        ...
        axios.post('/api/auth/confirmPhone',{email:this.getData.email, role: this.getData.role, code: data.phoneCode})
            .then((response)=>{
    ...
    computed:{
      getData(){
        return this.$store.getters['modals/phoneCode/getData'];
      }
    },
    destroyed() {
      this.animaStyle='';
